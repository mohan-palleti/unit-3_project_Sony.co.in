...
async function get() {
  try {
    //http://127.0.0.1:5000/api/mp3players
    let responce = await fetch(`https://sonyproducts.herokuapp.com/mp3player`, {
      method: "GET",
      headers: {
    ...
    let mdata = data.mp3player;

    arr = mdata;

    // localStorage.setItem("musicp", JSON.stringify(mdata));
    appendhead(mdata);
  } catch (err) {
    console.log(err);
  }
